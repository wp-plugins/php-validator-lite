...
if (class_exists("PhpValidator")) {
  ...
  class PhpValidator {
    ...
    static function validToken($token) {
      ...
      $valid = false;
      if (is_array($token) && in_array($token[0], $validCodes)) {
        $valid = true;
      }
      else if ($token == '(' || $token == ')') {
        $valid = true;
    ...
    function printAdminPage() {
        ...
        if (!$html->inError()) {
          ...
          if ($_POST['includeSelf']) {
            foreach ($sources as $s) {
              $testPHP .= "include_once '{$this->dirName}/$s' ;\n";
            }
          }
      ...
      $sourcePrompt = "List your php source filess here (comma separated).<br /><small>Giving multiple source files will make the line numbers meaningless. Current location is {$this->dirName}. Specify files relative to this location, or as full pathnames.</small>";
      ...
      if (empty($_POST['includeSelf'])) {
        $checked = '';
      }
      else {
        $checked = "checked='checked'";
      }
      printf("<input type='checkbox' $checked name='includeSelf'> &nbsp; Include the files in the autogenerated code? Check it if the file being validated is an include file with class definitions.<br /><span style='width:30px;display:inline-block'></span><small>[If it generates HTML output, it will probably interfere with the validation output.]</small><br \>");
      ...
      printf("<input type='checkbox' $checked name='showOutput'> &nbsp; Show methods, functions, includes etc. that are detected and validated. <br /><span style='width:30px;display:inline-block'></span><small>[Validated methods are always listed.]</small><br \>");
      if (empty($_POST['showEval'])) {
        $checked = '';
      }
      else {
        $checked = "checked='checked'";
      }
      printf("<input type='checkbox' $checked name='showEval'> &nbsp; Show the autogenerated code used for validation. <br /><span style='width:30px;display:inline-block'></span><small>[For debug purposes.]</small><br />");
      if (empty($_POST['showTokens'])) {
        $checked = '';
      }
      else {
        $checked = "checked='checked'";
...
if (class_exists("PhpValidator")) {
  ...
  if (isset($phpValidator)) {
    ...
    function phpValidator_admin_init() {
      global $phpValidator;
      wp_register_style('phpValidatorCSS', "{$phpValidator->URL}/ezpp.css");
    }

    function phpValidator_admin_menu() {
      global $phpValidator;
      $page = add_submenu_page('tools.php', 'PHP Validator', 'PHP Validator', "install_plugins", __FILE__, array($phpValidator, 'printAdminPage'));
