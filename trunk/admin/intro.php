...
if (!empty($no_visible_elements)) {
  ?>
  <a href="#" class="btn btn-warning goPro" style="float:right" data-toggle="tooltip" title="Get the Pro Version Now for $20.95!"> <i class="glyphicon glyphicon-shopping-cart"></i> Buy PHP Validator Pro Now!</a>
  <?php
}
?>
<h2>PHP Validator <br>
...
?>
<p><em>PHP Validator</em> is a smart and useful developer tool, and a quality checker for the end user. It scans the applications and plugins you specify for undefined functions and methods.</p>

<h4>Features</h4>

<ol>
  <li>Modern Admin Interface: <em>PHP Validator</em> sports a modern and beautiful admin interface based on the twitter bootstrap framework.</li>
  <li>Admin Interface Tour: A slick tour will take you around the admin page and familiarize you with its features.</li>
  <li>Generous Help: Whenever you need help, the information and hint is only a click away in <em>PHP Validator</em>. (In fact, it is only a mouseover away.)</li>
  <li>Robust Security: Unbreakable authentication (using hash and salt), impervious to SQL injection etc.</li>
  <li>WordPress Integration: PHP Validator comes with built-in WordPress integration. It works as a WordPress plugin if uploaded to the <code>wp-content/plugins</code> folder of your blog. What's more, you can switch to the standalone mode from the WordPress plugin admin page of this application, while still using the WordPress authentication mechanism and database.</li>
</ol>

<p><em>PHP Validator</em> is available as a freely distributed <a href="http://buy.thulasidas.com/lite/php-validator-lite.zip" title="Get PHP Validator Lite">lite version</a> and a <a href="http://buy.thulasidas.com/php-validator" title="Get PHP Validator Pro for $4.95" class="goPro">Pro version</a>, which adds a couple of extra features.</p>
